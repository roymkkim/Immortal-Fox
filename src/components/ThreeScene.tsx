...
const ThreeScene = forwardRef<ThreeSceneHandle, ThreeSceneProps>(({ isDarkMode, externalModelUrl, onMushroomCollect, initialMushroomCount = 0 }, ref) => {
  ...
  useEffect(() => {
    ...
    if (isMoving) {
          ...
          const maxSpeed = isGiant ? 420 : 135;  // CHANGED: slightly faster than 300/90
        ...
        const baseNemesisSpeed = velocity.current.length(); // CHANGED: match fox speed
        // Faster as you collect more mushrooms (difficulty scaling)
        const currentNemesisSpeed = baseNemesisSpeed; // CHANGED: no scaling, exact match
