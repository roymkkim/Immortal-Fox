...
const ThreeScene = forwardRef<ThreeSceneHandle, ThreeSceneProps>(
  ({ isDarkMode, externalModelUrl, onMushroomCollect, initialMushroomCount = 0 }, ref) => {
    ...
    const animations = useRef<{
      survey: THREE.AnimationAction | null;
    ...
    const spawnNemesis = (scene: THREE.Scene) => {
      ...
      const angle = Math.random() * Math.PI * 2;
      // Spawn nemesis slightly further out to account for higher speed (kept as-is)
    ...
    useEffect(() => {
      ...
      for (let i = 0; i < 400; i++) {
        ...
        const trunk = new THREE.Mesh(
          new THREE.CylinderGeometry(0.8, 1.2, trunkH),
          new THREE.MeshStandardMaterial({ color: 0x3d2b1f })
        );
        trunk.position.y = trunkH / 2;
        tree.add(trunk);
        const leaves = new THREE.Mesh(
          new THREE.ConeGeometry(8, trunkH * 2, 8),
          new THREE.MeshStandardMaterial({ color: LEAF_COLOR })
        );
      ...
      const loadChar = async () => {
        try {
          ...
          model.traverse((o) => {
            if ((o as any).isMesh) (o as any).castShadow = true;
          });
      ...
      const animate = () => {
        ...
        if (mistGroupRef.current) {
          mistGroupRef.current.children.forEach((m, i) => {
            ...
            if (m instanceof THREE.Mesh) (m.material as THREE.MeshBasicMaterial).opacity = 0.08 + Math.sin(elapsed * 0.5 + i) * 0.04;
          });
        }
        ...
        if (charGroupRef.current) {
          ...
          if (isMoving) {
            ...
            const moveVec = inputDir
              .clone()
              .normalize()
              .applyAxisAngle(new THREE.Vector3(0, 1, 0), mouseRotation.current.yaw);

            if (holdDuration > runThreshold) {
              fadeTo(animations.current.run);
              velocity.current.add(moveVec.multiplyScalar(maxSpeed * 1.8 * delta));
            } else {
              fadeTo(animations.current.walk);
              velocity.current.add(moveVec.multiplyScalar(maxSpeed * delta));
            }
          ...
          // === NEMESIS PURSUIT LOGIC (CHANGED) ===
          // Nemesis speed = Fox speed (based on the fox's actual current velocity)
          const nemesisToRemove: number[] = [];
          const foxSpeed = velocity.current.length(); // "units / second-ish" in your current system
          const currentNemesisSpeed = foxSpeed; // EXACT match to fox
          ...
          nemesisMushroomsRef.current.forEach((n, i) => {
            if (!isGiant) {
              ...
              // In giant mode, they flee even faster (kept as-is)
          ...
          const camOffset = new THREE.Vector3(
            0,
            camH + Math.sin(mouseRotation.current.pitch) * camDist * 0.5,
            -Math.cos(mouseRotation.current.pitch) * camDist
          ).applyAxisAngle(new THREE.Vector3(0, 1, 0), mouseRotation.current.yaw);
      ...
      const onMouseMove = (e: MouseEvent) => {
        if (isDragging.current) {
          ...
          targetRotation.current.pitch = Math.max(
            -0.4,
            Math.min(0.8, targetRotation.current.pitch - (e.clientY - lastMousePos.current.y) * 0.008)
          );
          lastMousePos.current = { x: e.clientX, y: e.clientY };
        }
      };
